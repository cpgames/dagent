...
import { getManagerBranchName, getManagerWorktreePath, DEFAULT_MANAGER_CONFIG } from '../../shared/types/pool'
...
export class FeatureManagerPool extends EventEmitter {
    ...
    try {
      ...
      for (const wt of worktrees) {
        ...
        for (const [managerId, entry] of this.managers) {
          ...
          if (wt.path === expectedPath || wt.path.endsWith(`/${entry.info.branchName.replace('worktree-', '')}`)) {
  ...
  private setupManagerListeners(manager: FeatureManager): void {
    ...
    manager.on('merge:requested', (data) => {
      ...
      console.log(`[FeatureManagerPool] Merge requested for ${data.featureId} -> ${data.targetBranch}`)
   ...
   *
   * Worktree is created lazily when feature actually starts.
   */
  async assignFeature(featureId: string, targetBranch: string): Promise<FeatureAssignmentResult> {
    ...
    for (const [featureManagerId, entry] of this.managers) {
      if (entry.manager.isIdle()) {
        const position = entry.manager.enqueue(featureId, targetBranch)
        this.updateManagerInfo(featureManagerId)
        ...
        return {
          ...
          isQueued: position > 0,
          worktreePath: entry.info.worktreePath // May be null if not yet created
        }
      }
    }
    ...
    const position = smallestManager.manager.enqueue(featureId, targetBranch)
    ...
    return {
      ...
      queuePosition: position,
      isQueued: position > 0,
      worktreePath: smallestManager.info.worktreePath
    }
  }
